...
let mapEeg1, mapEeg2, mapEeg3, mapEeg4, mapBeta,mapBeta2;
let r,ix,y;
...
function draw() {
  ...
  mapBeta2 = map(float(beta2[x]), 0, 1, 100, 800);
...
class Agent {
  ...
  aktivePos() {
    ...
    if (!this.whichAgent) {
      ...
      r = map(int(mapBeta2), 0, 100, 0, agent_size) * noise(2 + frameCount * 0.007) * 1;
      ...
      r = map(int(mapBeta2), 50, 100, 0, agent_size) * noise(5 + frameCount * 0.007) * 1;
